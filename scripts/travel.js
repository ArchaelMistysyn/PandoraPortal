...
const travelSubmenus = {
    ...
    exploration: [
        { label: "Map Tier", threshold: 5 },
        { label: "MapName", threshold: 5 },
        { label: "AutoMap", threshold: 21 },
        { label: "Manifest" }
    ],
    mortal: [
...
function updateTravelSubmenu(plane, playerData, player_gear_score) {
    ...
    if (planeMessage != "") {
        ...
        travelMessage.innerText = planeMessage;
        travelImage.style.backgroundImage = `url('${planeImage}')`;
    } else {
        travelScreenB.style.display = "flex";
...
function checkMonument(id, imageLink, btn) {
    ...
    .then(data => {
        if (data.success) {
            ...
            travelScreenA.style.display = "flex";
            travelScreenB.style.display = "none";
            travelMessage.innerHTML = "<div class='highlight-text quest-title'>" + data.title + "</div><div class='style-line'></div><div>" + zone_msg["monument" + id] + "</div>";
            travelImage.style.backgroundImage = `url('${imageLink}')`;
            travelImage.style.aspectRatio = '1 / 1';
